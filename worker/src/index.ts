...
async function extractSignals(page: any) {
...
async function analyze(target: string, env: Env) {
  ...
  try {
    const context = await browser.newContext({
      userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/142 Safari/537.36 GetSetGo/0.1',
    });
    ...
    return {
      ...
      policy: { vision: 'off-by-default', rawDomReturned: false, screenshotEmbedded: false },
      breakpoints,
    };
  } finally {
    await browser.close();
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (reqUrl.pathname === '/health') {
      return json(env, { ok: true, service: 'get-set-go-api', browser: 'cloudflare-browser-run' });
    }
